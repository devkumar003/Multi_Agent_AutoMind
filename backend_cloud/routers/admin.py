...
async def generate_ai_testcases(challenge_id: str, db: Session = Depends(get_db), admin: User = Depends(get_admin_user)):
    return {"error": "AI testcase generation is disabled on the cloud backend. Run the local backend to use AI features.", "raw": ""}
...
def get_all_users(db: Session = Depends(get_db), admin: User = Depends(get_admin_user)):
    ...
    return [{"id": u.id, "clerk_id": u.clerk_id, "username": u.username, "email": u.email, "is_admin": u.is_admin} for u in users]
